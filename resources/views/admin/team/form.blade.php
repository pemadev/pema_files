...
<div class="max-w-2xl">
    <form method="POST"
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
                <div class="w-8 h-8 bg-pema-50 rounded-lg flex items-center justify-center">
                    <i class="fi fi-rs-document text-pema-500 text-sm"></i>
                </div>
                <h3 class="font-heading font-semibold text-gray-900 text-sm">Biodata</h3>
            </div>
            <div class="p-5 space-y-5">
                <!-- Biography -->
                <div>
                    <label for="biography" class="block text-sm font-medium text-gray-700 mb-1.5">Biografi <span class="text-gray-400 font-normal">(opsional)</span></label>
                    <textarea id="biography"
                              name="biography"
                              rows="6"
                              class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-pema-500/20 focus:border-pema-500 text-sm transition-all @error('biography') border-red-300 @enderror"
                              placeholder="Tuliskan profil/biografi lengkap yang akan tampil di halaman publik">{{ old('biography', $team->biography) }}</textarea>
                    <p class="mt-1 text-xs text-gray-400">Ini yang akan tampil di halaman detail publik.</p>
                    @error('biography')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
