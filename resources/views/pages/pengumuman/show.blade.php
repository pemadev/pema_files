...
.article-content p { @apply text-gray-600 leading-relaxed mb-4; }
.article-content ul,
.article-content ol { @apply text-gray-600 leading-relaxed mb-4 pl-6; }
.article-content ul { @apply list-disc; }
.article-content ol { @apply list-decimal; }
.article-content li { @apply mb-2; }
.article-content blockquote { @apply border-l-4 border-gold-500 pl-4 italic text-gray-500 my-6; }
.article-content a { @apply text-pema-500 hover:text-pema-600 underline; }
.article-content img { @apply rounded-xl my-6 w-full; }
</style>
@endpush
        ...
        <div class="grid lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2">
                ...
                @if($news->document_path)
                    <div class="animate-fade-in-up delay-150 mt-8 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-pema-100 rounded-xl flex items-center justify-center flex-shrink-0">
                                <i class="fi fi-rs-file text-pema-500 text-xl"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="font-medium text-gray-900 text-sm">Lampiran Dokumen</p>
                                <p class="text-xs text-gray-400 truncate">{{ $news->document_name }}</p>
                            </div>
                            <a href="{{ asset('storage/' . $news->document_path) }}"
                               target="_blank"
                               class="inline-flex items-center gap-2 px-5 py-2.5 bg-pema-500 hover:bg-pema-600 text-white text-sm font-medium rounded-xl transition-all duration-300 flex-shrink-0">
                    ...
                    <div class="flex flex-wrap items-center gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
                               ...
                               class="w-10 h-10 bg-blue-50 hover:bg-blue-100 rounded-xl flex items-center justify-center text-blue-600 transition-colors"
                            ...
                            <a href="https://twitter.com/intent/tweet?text={{ urlencode($news->title) }}&url={{ urlencode(url()->current()) }}"
                               target="_blank" rel="noopener noreferrer"
                               class="w-10 h-10 bg-sky-50 hover:bg-sky-100 rounded-xl flex items-center justify-center text-sky-600 transition-colors"
                               aria-label="Bagikan ke Twitter">
                                <i class="fi fi-brands-twitter text-lg"></i>
                            </a>
                            <a href="https://wa.me/?text={{ urlencode($news->title . ' - ' . url()->current()) }}"
                               target="_blank" rel="noopener noreferrer"
                               class="w-10 h-10 bg-green-50 hover:bg-green-100 rounded-xl flex items-center justify-center text-green-600 transition-colors"
                            ...
                            <button onclick="navigator.clipboard.writeText(window.location.href).then(() => { this.querySelector('span').textContent = 'Tersalin!'; setTimeout(() => { this.querySelector('span').textContent = 'Salin Link'; }, 2000); })"
                                    class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm text-gray-600 transition-colors">
            ...
            <aside class="lg:col-span-1">
                ...
                <div class="animate-fade-in-up delay-200 sticky top-28">
                    <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                        ...
                        @if($relatedNews->count() > 0)
                            <div class="space-y-5">
                                @foreach($relatedNews as $related)
                                    <a href="{{ route('pengumuman.detail', $related) }}"
                        ...
                        @else
                            <p class="text-gray-400 text-sm text-center py-6">Belum ada pengumuman lainnya.</p>
                        @endif
